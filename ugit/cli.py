...
def parse_args():
    ...
    diff_parser.set_defaults(func=diff_func)
...
def _print_commit(oid, commit, refs: list[str] = None):
...
def branch(args):
    if not args.name:
        ...
        for branch_ in base.iter_branch_names():
            prefix = '*' if branch_ == current else ' '
            print(f'{prefix} {branch_}')
    else:
